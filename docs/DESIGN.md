...
区别只是 ta 不睡觉、不抱怨、月薪是 LLM API 账单(Claude / DeepSeek / Gemini / OpenAI / 其它任选)。
...
roster login llm              # LLM 配置(provider/base_url/model/api_key)
roster login claude           # 旧路径,仅 Claude(向后兼容)
...
| **持久化** | SQLite | 单文件、零运维 |
| **配置** | YAML | 注释友好 |
| **AI 模型** | Anthropic Claude / OpenAI-compatible (DeepSeek / Gemini / OpenAI / xAI ...) | 多 provider 可切换,DeepSeek 比 Claude Sonnet 便宜 ~10× |
...
| 4. Module C: Issue close → Confluence | ✅ 完成 | `archive-issue` 手动 + takeover 接入,草稿模式 + Slack 通知 |
| 5. Module D: 告警聚合 → Slack | ✅ 完成 | `aggregate-alert`,纯模板无 AI,绝不归因 |
| 6.a 观察面板 | ✅ 完成 | `roster status`、`roster logs` (含 -f / 过滤 / --json) |
| 6.b Budget 跟踪 | ✅ 完成 | audit.Entry 加 token+cost,Claude 4.x 价格表,roster status 显示 MTD |
| 6.c Budget 阈值 stop 模式 | ✅ 完成 | takeover 启动验证 + 每事件检查 + 超限退出,30s TTL 缓存 |
| 7. Undercover Mode | ✅ 完成 | system prompt suffix + 4 模块输出 redact(秘密 + vendor + AI 套话) |
| 6.c+ Budget downgrade 模式 | ✅ 完成 | 超限时关 AI 不停 daemon:Module A 走机械,B/C 跳过,D 不受影响 |
| 6.d Webhook 模式 | ✅ 完成 | 内嵌 HTTP server + HMAC-SHA256;与 polling 互斥;复用同一 dispatch handler |
| 8. 容器化 + CI | ✅ 完成(v0.1.0) | Dockerfile multi-stage(~40MB)+ CI(vet/test/build) + release(goreleaser) + GHCR docker push |
| 9. 多 LLM provider | ✅ 完成(v0.2.0) | Anthropic / OpenAI-compatible(DeepSeek / Gemini / xAI / OpenAI / ...);5 层凭证解析;价格表内置 13 个 model |
| 10. Helm chart | ✅ 完成(v0.2.1) | charts/roster:Deployment + ConfigMap + Secret + PVC,可选 Service + Ingress;单 Pod(cursor 不并发);existingSecret 模式可对接 Sealed/External Secrets |
| 11. Slack slash command | ✅ 完成(v0.2.1) | `/roster status / sync-issue / review-pr / archive-issue`;HMAC-v0;模块异步 ack(避免 3 秒超时);单 repo 守卫;复用 webhook HTTP listener |
| docs. 双语 README(中 / en) | ✅ 完成(v0.2.0) | README.md(中)+ README.en.md(英),顶部互链;.goreleaser/.dockerignore 已适配 |
| 8. 容器化 + CI | ✅ 完成 | Dockerfile(40MB image)、CI(test+vet+build)、release(goreleaser)、GHCR docker push |
| 3. Module B | ⏳ | |
| 4. Module C | ⏳ | |
| 5. Module D | ⏳ | |
| 6. 打磨 | ⏳ | |
